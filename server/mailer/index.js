...
var extraTransports = {
  'stub-transport': require('nodemailer-stub-transport')
};
...
var renderTemplate = function(templateName, locals, cb) {
  ...
  var filter = glob.sync(path.join(templatesDir,templateName, '*filter.*'))[0];
  
    if (filter) {
            locals = require(filter)(locals);
        }
...
var sendMail = function(mailing) {
  ...
  var send = function(ops, html, text, callback) {
    ...
    transport.sendMail({
      ...
      to: ops.to,
      subject: ops.subject || 'mailSupport',
      html: html || ops.html,
      text: text || ops.text
    }, callback);
  }
  ...
  if (options.template) {
    renderTemplate(options.template, mailing, function(err, html, text, locals) {
      if (err) {console.log(err)} else{send(locals, html, text);};
